...
OperationFile::OperationFile(string x){
	FileLocation = x;
}
...
void OperationFile::printToScreen() {
	char ch;
	ifstream file;
...
string OperationFile::getFile() {
	string buffer;
	char c;
	ifstream in(FileLocation);
	if (!in) {
		cout << FileLocation << " not found";   exit(1);
	}
	while (in.get(c)) buffer += c;
	in.close();
	return buffer;
...
void OperationFile::printXMLToScreen(string tag) {
	bool stripOtherTags = true;
	string text = getFile(FileLocation);
	vector<string> all = getData(text, tag);
...
string OperationFile::ConcantenateText(string input) {
	char ch;
	string output = "";
	ifstream file;
	file.open(FileLocation, ios::in);
...
void OperationFile::WriteToFile(string input) {
	string out = ConcantenateText(FileLocation, input);
	OverwriteToFile(FileLocation, out);
}
