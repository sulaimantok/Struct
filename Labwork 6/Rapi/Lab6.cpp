...
int cekrules(petak input, list forbidden) {
	int bawah = 0, kanan = 0, atas = 0, kiri =0, output = 0;
	...
	if (maps[input.y][input.x].kiri == 1){
		petak temp = maps[input.y][input.x - 1];
		if (temp.status != 0 && findInForbidden(temp) == 0)
			kiri = 1;
		else
			maps[input.y][input.x].kiri = 0;
	}
	if (maps[input.y][input.x].atas == 1) {
		petak temp = maps[input.y-1][input.x];
		if (temp.status != 0 && findInForbidden(temp) == 0)
			atas = 1;
		else
			maps[input.y][input.x].atas = 0;
	}
